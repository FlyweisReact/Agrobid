...
const Inventory = () => {
  ...
  const fetchData = async () => {
    try {
      const { data } = await axios.get(
        "https://ajeet-backend-new.vercel.app/api/v1/items"
      );
      setData(data);
    } catch (e) {
      console.log(e);
  ...
  function AddCrop(props) {
    ...
    const [varietyArray, setVarityArray] = useState([]);
    ...
    function handleAddButtonClick2() {
      setVarityArray((prevArray) => [...prevArray, variaty]);
      setVariety("");
        ...
        <Modal.Body>
          <Form onSubmit={postHandler}>
            ...
            <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
              ...
              <img
                src="https://www.pngarts.com/files/3/Plus-Symbol-PNG-Image-with-Transparent-Background.png"
                alt=""
  ...
  const deleteHandler = async (id) => {
    try {
      const { data } = await axios.delete(
        `https://ajeet-backend-new.vercel.app/api/v1/items/${id}`
      );
      console.log(data);
