...
const Inventory = () => {
        ...
        <Modal.Body>
          <Form onSubmit={postHandler}>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control type="text" onChange={(e) => setGrade(e.target.value)} />
            </Form.Group>
            <Form.Group className="mb-3">
              <Form.Label>Variety</Form.Label>
              <Form.Control type="text" onChange={(e) => setVariety(e.target.value)} />
            </Form.Group>
            <Form.Group className="mb-3">
              <Form.Label>Moisture</Form.Label>
              <Form.Control type="text" onChange={(e) => setMoisture(e.target.value)} />
            </Form.Group>
            <Form.Group className="mb-3">
              <Form.Label>Extraneous Matter </Form.Label>
              <Form.Control type="text" onChange={(e) => setExtraneous(e.target.value)} />
            </Form.Group>
            <Form.Group className="mb-3">
              <Form.Label>Foreign Matter</Form.Label>
              <Form.Control type="text" onChange={(e) => setForign(e.target.value)} />
            </Form.Group>
  ...
  const deleteHandler = async (id) => {
    try {
      const { data } = await axios.delete(`https://djqtflksic.execute-api.ap-south-1.amazonaws.com/dev/items/${id}`)
      console.log(data)
          ...
          <tbody>
            {filterData?.map((i, index) => (
              <tr key={index}>
                ...
                <td> {i.name} </td>
                <td> {i.variaty?.map((i , index))} </td>
                <td> {i.grade?.map((i , index) => (
