...
const Location = () => {
    ...
    function AddRatesModal(props) {
      ...
      const postHandler = async (e) => {
        e.preventDefault()
        try{
          const { data } = await axios.post("https://djqtflksic.execute-api.ap-south-1.amazonaws.com/dev/location/" ,{
            address , 
            state , 
            distract , 
            code
     ...
     const deleteHandler = async (id) => {
      try{
        const { data } = await axios.delete(`https://djqtflksic.execute-api.ap-south-1.amazonaws.com/dev/location/${id}`)
        console.log(data)
