...
const BuyerBid = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const { data } = await axios.get(
        `https://ajeet-backend-new.vercel.app/api/v1/placebid/getbuyer/${id}`
      );
    ...
    const editHandler = async (e) => {
      ...
      try {
        const { data } = await axios.put(
          `https://ajeet-backend-new.vercel.app/api/v1/placebid/update/${bidId}`,
          {
            quantity,
          }
        );
  ...
  function MyVerticallyCenteredModal3(props) {
    ...
    const fetchTransacation = async () => {
      try {
        const { data } = await axios.get(
          `https://ajeet-backend-new.vercel.app/api/v1/createbid/payment/placeBid/${bidId}`
        );
        setTransData(data);
