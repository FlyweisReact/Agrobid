...
const Products = () => {
  ...
  const fetchHandler = useCallback(async () => {
    try {
      const { data } = await axios.get(
        `https://ajeet-backend-new.vercel.app/api/v1/createbid/${role}`
      );
      setData(data);
      console.log(data);
    } catch (e) {
  ...
  const fetchSingleBid = async (lotId) => {
    try {
      const { data } = await axios.post(
        `https://ajeet-backend-new.vercel.app/api/v1/createbid/lotId`,
        {
          lotId,
        }
      );
    ...
    const putHandler = async (e) => {
      ...
      try {
        const { data } = await axios.put(
          `https://ajeet-backend-new.vercel.app/api/v1/createbid/verifyByAdmin/${id}`,
          { expiretime }
        );
        console.log(data);
  ...
  function BidModal(props) {
    ...
    const fetchBidder = useCallback(async () => {
      try {
        const { data } = await axios.post(
          `https://ajeet-backend-new.vercel.app/api/v1/createbid/waitlist/bid/${id}`,
          {
            user: role,
          }
        );
    ...
    const acceptBid = async () => {
      try {
        const { data } = await axios.post(
          `https://ajeet-backend-new.vercel.app/api/v1/createbid/accept/:id/:user/:bidcreatebid/accept/:id/:user/:bid`,
  ...
  function MyVerticallyCenteredModal3(props) {
    ...
    const fetchTransacation = async () => {
      try {
        const { data } = await axios.get(
          `https://ajeet-backend-new.vercel.app/api/v1/createbid/payment/bidId/${id}`
        );
        setTransData(data);
