    ...
    : data?.data?.filter(
      ...
      function MyVerticallyCenteredModal3(props) { 
        ...
        const fetchTransacation = async () => {
          try{
            const { data } = await axios.get(`https://djqtflksic.execute-api.ap-south-1.amazonaws.com/dev/createbid/payment/bidId/${id}`)
            setTransData(data)
            console.log(data)
          }catch(e) {
            console.log(e)
        ...
        return (
          ...
          >
            ...
            <Modal.Body>
              ...
              {
                data?.result?.length === 0 ? :     <div style={{width : '100%' , overflow  : 'auto'}}>
              <Table striped bordered hover >
                <thead>
                  <tr>
                    <td>SNo.</td>
                ...
                {filterData?.map((i, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <Button onClick={() => {
                        setId(i._id)
                        setModalShow2(true)}} >View</Button>
                    </td>
                    <td>
                      <i
