...
const Notify = () => {
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const postHandler = async (e) => {
      e.preventDefault()
      try{
        const { data } = await axios.post("https://djqtflksic.execute-api.ap-south-1.amazonaws.com/dev/notify" , {message})
        console.log(data)
        toast.success("Added")
        fetchData()
        props.onHide()
        ...
        <Modal.Body>
          <Form onSubmit={postHandler} >
            <FloatingLabel
              ...
              className="mb-3"
            >
              <Form.Control as="textarea" placeholder="Leave a comment here" onChange={(e) => setMessage(e.target.value)} />
            </FloatingLabel>
