  ...
  const fetchData = async () => {
    try {
      const { data } = await axios.get(
        "https://ajeet-backend-new.vercel.app/api/v1/inspaction/all"
      );
    ...
    const postHandler = async (e) => {
      ...
      try {
        const { data } = await axios.post(
          `https://ajeet-backend-new.vercel.app/api/v1/inspaction/add`,
    ...
    const putHandler = async (e) => {
      ...
      try {
        const { data } = await axios.put(
          `https://ajeet-backend-new.vercel.app/api/v1/inspaction/update/${id}`,
          { status }
        );
        console.log(data);
  ...
  const deleteHandler = async (id) => {
    try {
      const { data } = await axios.delete(
        `https://ajeet-backend-new.vercel.app/api/v1/inspaction/delete/${id}`
      );
      console.log(data);
  ...
  const filterData = !query
    ...
    : data?.filter(
        (i) =>
          ...
          i?.price?.toString()?.toLowerCase().includes(query?.toLowerCase())
      );
